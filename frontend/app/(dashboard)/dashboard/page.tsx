...
export default function Home() {
  ...
  // State untuk simpan angka CPU & RAM
  const [stats, setStats] = useState({ cpu: 24, ram: 32 });

  // Interval biar angka CPU & RAM berubah tiap 3 detik
  useEffect(() => {
    const timer = setInterval(() => {
      setStats({
        cpu: Math.floor(Math.random() * (35 - 20 + 1)) + 20, // Angka acak 20-35
        ram: Math.floor(Math.random() * (40 - 30 + 1)) + 30  // Angka acak 30-40
      });
    }, 3000);

    return () => clearInterval(timer); // Bersihin memory kalau pindah halaman
  }, []);
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-slate-500 font-inter">Selamat datang kembali, Arif. Berikut ringkasan data hari ini.</p>
      </div>
